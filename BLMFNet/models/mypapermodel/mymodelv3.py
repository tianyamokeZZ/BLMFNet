...
from models.MCANet.mymodule import ESM, BasicConv2d, ESM_small, CAM_Module, PAM_Module, AFF
from models.mypapermodel.mymodule import BasicConv2d, BSM
...
class ZZ0Net(nn.Module):
    def __init__(self, num_classes, atrous_rates=[6,12,18], channel2=256, channel1=128 ,channel0=64, num_class=1):
        ...
        # self.feature_boundary = nn.Sequential(
        ...
        # self.boundary = BSM(all_channel=256)
    ...
    def forward(self, sar, opt):
        ...
        # low_high_boudary = self.feature_boundary(low_high_feature)
        # boundary = self.boundary(low_high_boudary)  # 这里boundary 返回两个 第二个是boundary
        return seg, loc_seg
...
class ZZNet(nn.Module):
    def __init__(self):
        ...
        state_dict = {k.replace('module.', ''): v for k, v in torch.load("/home/omnisky/ZZ/DFCtrack2/DFCtrack2_for_experiment/my_model_code_1/stage1.pth").items()}
...
class ZZ1Net(nn.Module):
    def __init__(self):
        ...
        self.a_param = nn.Parameter(torch.zeros(1))

    def forward(self, sar, opt):
        coarse_seg, locseg, coarse_boundary = self.fusion(sar, opt)
        boundary = self.boundaryextraction(sar, opt)
        final_seg = coarse_seg + coarse_seg * boundary * self.a_param
        return final_seg
