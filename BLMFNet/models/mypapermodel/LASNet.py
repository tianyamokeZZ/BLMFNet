...
class ESM(nn.Module):
    def __init__(self, all_channel=64):
        ...
        self.pred = nn.Conv2d(all_channel, 1, kernel_size=3, padding=1, bias = True)
...
if __name__ == '__main__':  # 本来输出的edge为2通道 mask为两通道 语义为class binary为2 现在全为1
    rgb = torch.randn(4, 3, 512, 512).to(device='cuda:0')
    sar = torch.randn(4, 1, 512, 512).to(device='cuda:0')
    net = LASNet(n_classes=2).to(device='cuda:0')
    out = net(rgb, sar)
    print(out[0].shape)
    print(out[1].shape)
    print(out[2].shape)
