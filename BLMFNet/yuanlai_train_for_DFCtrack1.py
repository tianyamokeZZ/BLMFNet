...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.model == 'base':
            from models.MCANet.mcanet1 import MCANet
            model = MCANet(num_classes=1)
            print('======> model unet base =============== ')
        if args.model == 'unetsar':
            from models.u_net_sar import UNet
            model = UNet(num_classes=1)
            print('======> model unet sar =============== ')
        if args.model == 'cmgfnet':
            from models.CMGFNet import FuseNet
            model = FuseNet(num_classes=1)
            print('======> model cmgfnet =============== ')
    ...
    def training(self, epoch):
        ...
        for index, data in enumerate(tbar):
            ...
            # outputs, hah, hah1 = self.model(imgs_opt, imgs_sar) # cmgfnet
            # outputs, locloss = self.model(imgs_sar, imgs_opt) # zzonet
            outputs = self.model(imgs_sar, imgs_opt)  # base
            semantic = outputs
            ...
            loss = dic_loss + bce_loss # cmgfnet
        ...
        torch.save(self.model.state_dict(), " base.pth")
